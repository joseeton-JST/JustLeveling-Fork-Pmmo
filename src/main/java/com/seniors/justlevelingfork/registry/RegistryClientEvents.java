        ...
        if ((Minecraft.getInstance()).player != null && (Minecraft.getInstance()).player.isAlive()) {
                    ...
                    if (!aptitudesList.isEmpty()) {
                        ...
                        for (Aptitudes aptitudes : aptitudesList) {
                            ...
                            if (aptitude != null) {
                                ...
                                tooltips.add(Component.translatable("tooltip.aptitude.item_requirements", Component.literal(aptitude.getDisplayNameOrFallback()).withStyle(ChatFormatting.DARK_AQUA), Component.literal(String.valueOf(aptitudes.getAptitudeLvl())).withStyle(colour)).withStyle(ChatFormatting.GRAY));
                            }
                        }
                    }
                    return;
            ...
            if (list != null) {
                ...
                for (Aptitudes aptitudes : list) {
                    ...
                    if (aptitude != null) {
                        ChatFormatting colour = (AptitudeCapability.get().getAptitudeLevel(aptitude) >= aptitudes.getAptitudeLvl()) ? ChatFormatting.GREEN : ChatFormatting.RED;
                        tooltips.add(Component.translatable("tooltip.aptitude.item_requirements", Component.literal(aptitude.getDisplayNameOrFallback()), Component.literal(String.valueOf(aptitudes.getAptitudeLvl())).withStyle(colour)));
                    }
                }
            }
        }
