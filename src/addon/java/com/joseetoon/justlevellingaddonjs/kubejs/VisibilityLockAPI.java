...
public class VisibilityLockAPI {
    ...
    public static void addVisibilityLock(String aptitude, Object... requirements) {
        ...
        if (normalizedRequirements.isEmpty()) {
            removeVisibilityLock(normalizedAptitude);
            return;
        }

        Map<String, Integer> merged = new LinkedHashMap<>(VISIBILITY_LOCKS.getOrDefault(normalizedAptitude, Collections.emptyMap()));
        normalizedRequirements.forEach((requiredAptitude, requiredLevel) ->
                merged.merge(requiredAptitude, requiredLevel, Math::max));
        VISIBILITY_LOCKS.put(normalizedAptitude, merged);
    }
